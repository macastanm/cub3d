...
void	my_mlx_pixel_put_mini(t_data *data, int x, int y, int color)
{
	...
	dst = data->mini->mini->addr + (y * data->mini->mini->line_length + x * (data->mini->mini->bpp / 8));
...
void	draw_pixel(t_data *data, int color)
{
	...
	while (i < 13)
	{
		...
		while (j < 13)
		{
			my_mlx_pixel_put_mini(data, (data->mini->x + j), (data->mini->y + i), color);
			j++;
		}
		i++;
	}
